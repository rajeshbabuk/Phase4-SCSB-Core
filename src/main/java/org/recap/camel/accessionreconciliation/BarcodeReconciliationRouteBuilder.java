...
public class BarcodeReconciliationRouteBuilder extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("aws-s3://{{scsbBucketName}}?prefix="+accessionReconciliationS3Dir + imsLocation + "/" + institution + "/{{s3DataFeedFileNamePrefix}}&deleteAfterRead=false&sendEmptyMessageWhenIdle=true&autocloseBody=false&region={{awsRegion}}&accessKey=RAW({{awsAccessKey}})&secretKey=RAW({{awsAccessSecretKey}})")
                .routeId(imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_S3_ROUTE_ID)
                .noAutoStartup()
                .choice()
                .when(gzipFile)
                .unmarshal().gzipDeflater()
                .log(imsLocation + " - " + institution + " Accession Reconciliation FTP Route Unzip Complete")
                .process(new StartRouteProcessor(imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_DIRECT_ROUTE))
                .to(ScsbConstants.DIRECT + imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_DIRECT_ROUTE)
                .when(body().isNull())
                .log("No File To Process For " + imsLocation + " - " + institution + " Accession Reconciliation")
                .process(new StopRouteProcessor(imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_S3_ROUTE_ID))
                .otherwise()
                .process(new StartRouteProcessor(imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_DIRECT_ROUTE))
                .to(ScsbConstants.DIRECT + imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_DIRECT_ROUTE)
                .endChoice();

        from(ScsbConstants.DIRECT + imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_DIRECT_ROUTE)
                .routeId(imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_DIRECT_ROUTE)
                .noAutoStartup()
                ...
                .bean(applicationContext.getBean(AccessionReconciliationProcessor.class, institution, imsLocation), ScsbConstants.PROCESS_INPUT)
                .end()
                .onCompletion()
                .process(new StopRouteProcessor(imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_DIRECT_ROUTE));
        from(ScsbConstants.DAILY_RR_FS_FILE + accessionReconciliationFilePath + "/" + imsLocation + "/" + institution + ScsbConstants.DAILY_RR_FS_OPTIONS)
                .routeId(imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_FS_ROUTE_ID)
                .noAutoStartup()
                ...
                .onCompletion()
                .bean(applicationContext.getBean(AccessionReconciliationEmailService.class, institution, imsLocation), ScsbConstants.PROCESS_INPUT)
                .process(new StopRouteProcessor(imsLocation + institution + ScsbConstants.ACCESSION_RECONCILIATION_FS_ROUTE_ID))
